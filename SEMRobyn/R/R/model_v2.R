...
robyn_train <- function(InputCollect, hyper_collect,
                        ...
                        SEM_mod = NULL,robyn_object = NULL) {
      ...
      system.time({
        for (lng in 1:iterNG) { # lng = 1
          ...
          robyn_iterations <- function(i, ...) { # i=1
            ...
            if (use_SEM==TRUE) {
              x_train <- dt_window %>% select(-dep_var)
              
              sem_mod <- regsem_model(path = SEM_mod, 
                                      data = dt_window, 
                                      lambda_scaled = lambda_scaled, 
                                      out_dir = robyn_object,
                                      out_label = paste(trial, lng, i, sep = "_"))
              ...
              coef <- path_summary(mod_out$coef_dt, cols = colnames(x_train))
              ...
              nrmse <- ifelse(ts_validation, mod_out$nrmse_val, mod_out$nrmse_train) #mod_out$nrmse_train
              mape <- 0
...
path_summary <- function(
  ...
  path_df <- coef_list %>% as.data.frame() %>% t()
  items_df <- as.character(col_list) %>% data.frame()
  paths_df <- cbind(items_df,path_df)
  colnames(paths_df) <- c("items", "coefs")
  path_df_grouped <- paths_df %>% group_by(items) %>% summarise(s0 = sum(coefs))
  # out_path <- as.numeric(coef_list) %>% as.matrix()
  out_path <- as.numeric(path_df_grouped$s0) %>% as.matrix()
  # rownames(out_path) <- as.character(col_list)
  rownames(out_path) <- as.character(path_df_grouped$items)
